...
serve(async (req) => {
  ...
  try {
    ...
    if (tradersError) {
      ...
      throw new Error('Failed to fetch active traders');
    }
    ...
    if (!activeTraders || activeTraders.length === 0) {
      ...
      return new Response(
        JSON.stringify({ 
          success: true, 
          message: 'No active traders to generate signals for',
          activeTraders: 0
        }),
        { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    ...
    // Store signals in database for each active trader
    if (allSignals.length > 0) {
      console.log(`Storing ${allSignals.length} signals for ${activeTraders.length} active traders...`);

      // Create signals for each active trader
      const formattedSignals = activeTraders.flatMap(trader => 
