...
const POLYGON_API_KEY = import.meta.env.VITE_POLYGON_API_KEY || 'YOUR_API_KEY';
...
export function useStockDetails(symbol: string) {
  return useQuery({
    ...
    enabled: !!symbol,
    staleTime: 24 * 60 * 60 * 1000, // Company details don't change often
  });
}
...
export function useStockNews(symbol: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `https://api.polygon.io/v2/reference/news?ticker=${symbol.toUpperCase()}&limit=10&apiKey=${POLYGON_API_KEY}`
      );
    ...
    enabled: !!symbol,
    staleTime: 5 * 60 * 1000, // News cached for 5 minutes
  });
}
